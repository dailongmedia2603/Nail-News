...
export function CreatePostForm() {
  ...
  async function onSubmit(data: CreatePostFormValues) {
    ...
    if (data.images && data.images.length > 0) { /* ... image upload logic ... */ }
    ...
    <FormProvider {...formMethods}>
      <Form {...formMethods}>
        <form onSubmit={formMethods.handleSubmit(onSubmit)} className="space-y-8">
          ...
          {formMethods.watch("category") === "Bán tiệm" && ( <div className="space-y-8 p-6 border rounded-lg"> {/* ... Bán tiệm fields ... */} </div> )}
          {formMethods.watch("category") === "Cần thợ" && ( <div className="space-y-8 p-6 border rounded-lg"> {/* ... Cần thợ fields ... */} </div> )}
